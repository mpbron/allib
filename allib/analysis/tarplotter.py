...
from typing import Any, Dict, FrozenSet, Generic, Iterable, List, Mapping, Optional, Sequence, Tuple, TypeVar, Union

import matplotlib.pyplot as plt
import numpy as np

from allib.estimation.rasch_multiple import ModelStatistics
...
def filter_model_infos(mapping: Mapping[str,AbstractEstimator]) -> Mapping[str, ModelStatistics]:
     results = {key: est.model_info[-1] for key, est in mapping.items() if hasattr(est, "model_info")} # type: ignore 
     return results
    ...
    def _plot_estimators(self, included_estimators: Iterable[str] = list()) -> None:
        ...
        for estimator in included_estimators:
            if estimator in self.model_stats[self.it]:
                deviances = [self.model_stats[it][estimator].deviance for it in self.it_axis]
                plt.plot(self._effort_axis(), deviances, label=f"Deviance {estimator}", linestyle="--")