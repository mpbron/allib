...
LVT = TypeVar("LVT")
PVT = TypeVar("PVT")
class MostCertainSampling(ProbabiltyBased[KT, DT, RT, LT]):
...
class MinEntropy(ProbabiltyBased[KT, DT, RT, LT]):
...
class MostConfidence(ProbabiltyBased[KT, DT, RT, LT]):
...
class LabelMaximizer(LabelProbabilityBased[KT, DT, RT, LT]):
