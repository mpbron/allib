...
```{r}
path <- "results/hall4/design_matrix_300.csv" 
df <- rasch.csv(path)
df
```
...
```{r}
source("./allib/estimation/multiple_estimate.R")
df.rc <- rcapture.df(df)
df.rc
res <- closedpMS.t(df.rc, T)
res
```
...
```{r}
df.onlypos <- df %>% filter(positive == 1) %>% select(starts_with("learner") & !ends_with("positive"), starts_with("h") & ends_with("positive"), matches("count"))
df.onlypos
```
...
Gegeven een ingevulde design-matrix met data gegenereerd door $l$ learners.
...
```{python}
from allib.estimation.rasch_multiple import rasch_estimate_bf_stats

bf_estimates, bf_deviances = rasch_estimate_bf_stats(df, 8911, 0.5)
bf_df = pd.DataFrame.from_dict({"estimate": bf_estimates[:,0], "deviance": bf_deviances})
bf_df
```

```{r}
ggplot(data=py$bf_df) + aes(x=estimate, y=deviance) + geom_point()
```
...
#### Methode met puntschatting met BruteForce, EM initialiasatie met parameters puntschattingsmodel

```{python}
from allib.estimation.rasch_multiple import rasch_estimate_bf_parametric
bf_parametric_results, bf_stats = rasch_estimate_bf_parametric(df, 8911)
bf_parametric_results
```

```{r}
ggplot() + aes(x=py$bf_stats$preds) +  geom_density()
```
