...
```{r}
path <- "results/halldataset/design_matrix_200.csv" 
df <- rasch.csv(path)
df
```
...
2.  In *M-step*, fit log-lineair model op 16 cellen voor hits en op 16 cellen voor niet-hits in *E-step*, gebruik fitted values uit *M-step* om 1.950 opnieuw te verdelen.
